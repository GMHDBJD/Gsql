...
{
  stream >> table_schema.root_page_id >> table_schema.max_id >> table_schema.column_order_vector >> table_schema.column_schema_map >> table_schema.primary_set >> table_schema.index_schema_map;
  return stream;
}

Stream &operator>>(Stream &stream, ColumnSchema &column_schema)
{
  stream >> column_schema.data_type >> column_schema.not_null >> column_schema.null_default >> column_schema.unique >> column_schema.index_name >> column_schema.default_value >> column_schema.reference_table_name >> column_schema.reference_column_name;
  return stream;
}
...
{
  return getSize(table_schema.root_page_id) + getSize(table_schema.max_id) + getSize(table_schema.column_order_vector) + getSize(table_schema.column_schema_map) + getSize(table_schema.primary_set) + getSize(table_schema.index_schema_map);
}

size_t getSize(const ColumnSchema &column_schema)
{
  return getSize(column_schema.data_type) + getSize(column_schema.not_null) + getSize(column_schema.null_default) + getSize(column_schema.unique) + getSize(column_schema.index_name) + getSize(column_schema.default_value) + getSize(column_schema.reference_table_name) + getSize(column_schema.reference_column_name);
}
